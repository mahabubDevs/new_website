...
export const Route = createFileRoute("/case-studies")({
  head: () => ({
    meta: [
      { title: "Case Studies — Software We've Built | Nexora Labs" },
      { name: "description", content: "Real results from real clients. Explore Nexora Labs case studies in fintech, health, logistics, e-commerce and more." },
      { property: "og:title", content: "Case Studies — Nexora Labs" },
        ...
        {cases.map((c, i) => (
          <Reveal key={c.title} delay={i * 60}>
            <article className="rounded-3xl border border-border bg-gradient-card p-8 lg:p-10 hover:border-primary/40 hover:shadow-glow transition-all">
              <div className={`aspect-[16/4] rounded-2xl bg-gradient-to-br ${c.color} mb-8 relative overflow-hidden border border-border`}>
                <div className="absolute inset-0 grid-pattern opacity-30" />
                <div className="absolute inset-0 grid place-items-center">
                  <span className="font-display text-6xl font-bold text-foreground/15">{c.tag}</span>
                </div>
              </div>

              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2">
                  <span className="inline-block text-xs font-semibold uppercase tracking-widest text-accent">{c.tag}</span>
                  <h2 className="mt-3 font-display text-3xl font-bold">{c.title}</h2>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {c.architecture.map((a) => (
                        <span key={a} className="px-3 py-1.5 rounded-full glass text-xs font-medium">{a}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 content-start">
                  {c.metrics.map((m) => (
                    <div key={m.l} className="rounded-2xl glass p-4 text-center hover:shadow-glow transition-shadow">
                      <m.icon className="h-5 w-5 text-primary-glow mx-auto" />
                      <p className="mt-2 font-display text-2xl font-bold text-gradient">{m.v}</p>
